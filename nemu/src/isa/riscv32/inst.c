...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, R(rd) = CSR(imm);
          CSR(imm) = src1);
  INSTPAT("??????? ????? ????? 010 00000 11100 11", csrrs, I, R(rd) = CSR(imm);
          printf("csrrs zero\n"););
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, I, R(rd) = CSR(imm);
          CSR(imm) = CSR(imm) | src1;);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, I,
          s->dnpc = do_ecall(s->pc););
  INSTPAT(
      "0011000 00010 00000 000 00000 11100 11", mret, I, s->dnpc = CSR(0x341);
      IFDEF(
          CONFIG_ETRACE, elf_func_info cur_func = find_func_by_addr(s->dnpc);
          etrace_log_write("---mret to %s[0x%0x]\n", cur_func.name, s->dnpc)););
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B,
          if ((int32_t)src1 == (int32_t)src2) s->dnpc = s->dnpc - 4 + imm);
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B,
          if (src1 < src2) s->dnpc = s->dnpc - 4 + imm);
  ...
  /******************** J ************************/
  /***********************************************/
  INSTPAT("??????? ????? ????? ??? ????? 1011 11", jal, J,
          IFDEF(CONFIG_FTRACE, ftrace(s, s->pc + imm));
          R(rd) = s->snpc; s->dnpc = s->dnpc - 4 + imm);
