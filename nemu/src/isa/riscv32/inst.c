...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
    ...
    INSTPAT("000???? ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
    INSTPAT("000???? ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
    INSTPAT("000???? ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2);
    INSTPAT("000???? ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << src2);

    INSTPAT("000???? ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (src1 < src2));
