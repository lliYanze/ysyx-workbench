...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
    ...
    INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (SEXT(src1, 32) >> imm));//算数右移

    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm));

    /*-------------------*/
    INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));
    INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16));//符号扩展
    ...
    INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));//零扩展
    /*-------------------*/
    ...
    /******************** B ************************/
    /***********************************************/
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if((int32_t)src1 == (int32_t)src2) s->dnpc = s->dnpc - 4 + imm);
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if((int32_t)src1 != (int32_t)src2) s->dnpc = s->dnpc - 4 + imm);
    INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((int32_t)src1 < (int32_t)src2) s->dnpc = s->dnpc - 4 + imm);
    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((int32_t)src1 >= (int32_t)src2) s->dnpc = s->dnpc - 4 + imm);
